...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  public PaginatedListTo<SpecialEntity> findSpecials(SpecialSearchCriteriaTo criteria) {
    ...
    if (activePeriod != null) {
      BooleanExpression sDay = Alias.$(special.getActivePeriod().getStartingDay()).eq(activePeriod.getStartingDay());
      BooleanExpression sHour = Alias.$(special.getActivePeriod().getStartingHour()).eq(activePeriod.getStartingHour());
      BooleanExpression eDay = Alias.$(special.getActivePeriod().getEndingDay()).eq(activePeriod.getEndingDay());
      BooleanExpression eHour = Alias.$(special.getActivePeriod().getEndingDay()).eq(activePeriod.getEndingDay());

      query.where(sDay.and(sHour).and(eDay).and(eHour));
